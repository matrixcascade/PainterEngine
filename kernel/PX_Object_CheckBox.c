...
px_void PX_Object_CheckBoxOnMouseMove(PX_Object *pObject,PX_Object_Event e,px_void *user_ptr)
{
	...
	if (pcb)
	{
		if(PX_ObjectIsCursorInRegion(pObject,e))
		{
			...
			{
				if (pcb->state!=PX_OBJECT_CHECKBOX_STATE_ONCURSOR)
				{
					PX_Object_Event e = {0};
		...
		{
			if (pcb->state!=PX_OBJECT_CHECKBOX_STATE_NORMAL)
			{
				PX_Object_Event e = {0};
...
PX_OBJECT_RENDER_FUNCTION(PX_Object_CheckBoxRender)
{
	...
	switch (pcb->state)
	{
	case PX_OBJECT_CHECKBOX_STATE_NORMAL:
		PX_GeoDrawRect(psurface,(px_int)objx,(px_int)objy,(px_int)objx+(px_int)objWidth-1,(px_int)objy+(px_int)objHeight-1,pcb->BackgroundColor);
		break;
	case PX_OBJECT_CHECKBOX_STATE_ONPUSH:
		...
		break;
	case PX_OBJECT_CHECKBOX_STATE_ONCURSOR:
