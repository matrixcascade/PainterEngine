...
#define PX_Object_SwitchEffect_open_close_time 300
#define PX_Object_SwitchEffect_hold_time 500


PX_OBJECT_RENDER_FUNCTION(PX_Object_SwitchEffectRender)
{
	PX_Object_SwitchEffect* pdesc = PX_ObjectGetDescIndex(PX_Object_SwitchEffect, pObject, 0);
	px_rect region = PX_ObjectGetRect(pObject);
	px_int x = (px_int)region.x;
	px_int y = (px_int)region.y;
	px_int sx, sy;
	for (sy = 0; sy < ((psurface->height + 127) >> 7); sy++)
	{
		for (sx = 0; sx < ((psurface->width + 127) >> 7); sx++)
		{
			px_int current_elasped = pdesc->elapsed - (px_int)PX_sqrt(sx * sx * 16384.f + sy * sy * 16384.f) / 2;
			if (current_elasped > 0)
			{
				if (current_elasped <= PX_Object_SwitchEffect_open_close_time)
				{
					px_float scale = (current_elasped * 1.f / PX_Object_SwitchEffect_open_close_time);
					px_int dx = sx * 128 + 64;
					px_int dy = sy * 128 + 64;
					px_int dw = (px_int)(128 * scale);
					px_int dh = (px_int)(128 * scale);
					PX_GeoDrawRect(psurface, dx - dw / 2, dy - dh / 2, dx + dw / 2, dy + dh / 2, pdesc->drawcolor);
				}
				else if (current_elasped <= PX_Object_SwitchEffect_open_close_time + PX_Object_SwitchEffect_hold_time)
				{
					px_int dx = sx * 128 + 64;
					px_int dy = sy * 128 + 64;
					PX_GeoDrawRect(psurface, dx - 64, dy - 64, dx + 64, dy + 64, pdesc->drawcolor);
				}
				else if (current_elasped <= PX_Object_SwitchEffect_open_close_time * 2 + PX_Object_SwitchEffect_hold_time)
				{
					px_float scale = (PX_Object_SwitchEffect_open_close_time * 2 + PX_Object_SwitchEffect_hold_time - current_elasped) * 1.f / PX_Object_SwitchEffect_open_close_time;
					px_int dx = sx * 128 + 64;
					px_int dy = sy * 128 + 64;
					px_int dw = (px_int)(128 * (scale));
					px_int dh = (px_int)(128 * (scale));
	...
	if ((px_int)pdesc->elapsed >= PX_Object_SwitchEffect_open_close_time * 2 + PX_Object_SwitchEffect_hold_time + (px_int)PX_sqrt(psurface->width * psurface->width * 1.f + psurface->height * psurface->height))
	{
		PX_ObjectDelayDelete(pObject);
	}
...
PX_Object* PX_Object_SwitchEffectCreate(px_memorypool* mp, PX_Object* parent, px_color clr)
{
	PX_Object* pObject = PX_ObjectCreateEx(mp, parent, 0, 0, 0, 0, 0, 0, 0, 0, PX_Object_SwitchEffectRender, 0, 0, sizeof(PX_Object_SwitchEffect));
	PX_Object_SwitchEffect* pdesc = PX_ObjectGetDescIndex(PX_Object_SwitchEffect, pObject, 0);
	if (pObject == PX_NULL)
	{
		return PX_NULL;
...
#define PX_Object_SwitchEffect2_open_close_time 600
PX_OBJECT_RENDER_FUNCTION(PX_Object_SwitchEffect2Render)
{
	PX_Object_SwitchEffect* pdesc = PX_ObjectGetDesc(PX_Object_SwitchEffect, pObject);
	if (pdesc->elapsed< PX_Object_SwitchEffect2_open_close_time)
	{
		px_color clr = pdesc->drawcolor;
		clr._argb.a = (px_uchar)(255 * (pdesc->elapsed*1.0f / PX_Object_SwitchEffect2_open_close_time));
		PX_GeoDrawRect(psurface, 0, 0, psurface->width - 1, psurface->height - 1, clr);
	}
	else if (pdesc->elapsed < PX_Object_SwitchEffect2_open_close_time*2)
	{
		px_color clr = pdesc->drawcolor;
		clr._argb.a = 255-(px_uchar)(255 * (pdesc->elapsed - PX_Object_SwitchEffect2_open_close_time*1.f) / PX_Object_SwitchEffect2_open_close_time);
		PX_GeoDrawRect(psurface, 0, 0, psurface->width - 1, psurface->height - 1, clr);
	}
	else
	{
...
PX_Object* PX_Object_SwitchEffect2Create(px_memorypool* mp, PX_Object* parent)
{
	PX_Object* pObject = PX_ObjectCreateEx(mp, parent, 0, 0, 0, 0, 0, 0, 0, 0, PX_Object_SwitchEffect2Render, 0, 0, sizeof(PX_Object_SwitchEffect2));
	PX_Object_SwitchEffect2* pdesc = PX_ObjectGetDescIndex(PX_Object_SwitchEffect2, pObject, 0);
	if (pObject == PX_NULL)
	{
		return PX_NULL;
	}
	pdesc->drawcolor = PX_COLOR_BLACK;
	return pObject;
}

px_void PX_Object_SwitchEffect2SetColor(PX_Object* pObject, px_color clr)
{
	PX_Object_SwitchEffect2* pdesc = PX_ObjectGetDescIndex(PX_Object_SwitchEffect2, pObject,0);
	pdesc->drawcolor = clr;
}
